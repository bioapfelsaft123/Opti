...
####
# CHANGES MADE:
# - change the end of the input data
...
# Class for the robust stochastic model   ## Change Gen_N_Data_scenarios and N_S and P_N_opt in (), init__ and 2 time each
class InvestmentModel_Robust_test():
    def __init__(self, Parameters, Data, DA_Price, Gen_N_Data_scenarios, N_S, P_N_opt, Model_results = 1, Guroby_results = 1):
        self.D = Data  # Data
        self.P = Parameters  # Parameters
        self.DA_Price = DA_Price  # Day-ahead price
        self.Gen_N_Data_scenarios = Gen_N_Data_scenarios
        self.N_S = N_S
        self.P_N_opt = P_N_opt
        self.Model_results = Model_results  # Display results
        self.Guroby_results = Guroby_results  # Display guroby results
        self.var = Expando()  # Variables
        self.con = Expando()  # Constraints
    ...
    def _build_constraints(self):
        ...
        for s in range(self.N_S):
            self.con.budget = self.m.addConstr(gp.quicksum(self.var.P_N [g] * self.Gen_N_Data_scenarios[g,s] for g in range(self.P.N_gen_N)) <= self.P.B, name='Budget constraint')

        # Fix the P_N values
        self.con.fix_P_N = self.m.addConstr(self.var.P_N == self.P_N_opt, name='Fix P_N')


    def _build_objective(self):
        revenues = ((self.var.p_N @ self.D.Gen_N_Z.T) * self.DA_Price).sum()  # don't use quicksum here because it's a <MLinExpr (3600, N_zone)>
        op_costs = gp.quicksum(self.var.p_N @ self.D.Gen_N_OpCost)
        invest_costs = gp.quicksum((1/self.N_S)*self.var.P_N[g] * self.Gen_N_Data_scenarios[g,s] for g in range(self.P.N_gen_N) for s in range(self.P.N_S))
        objective = self.P.R*(revenues - op_costs) - invest_costs
        self.m.setObjective(objective, GRB.MAXIMIZE)
